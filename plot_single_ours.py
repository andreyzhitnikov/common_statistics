...
g = sns.heatmap(df, ax=axes[0], square=True, cbar=True, xticklabels=2,
                yticklabels=2)
g.set_title('Mean covariance - hands')

df = pd.DataFrame(data=mdm.covmeans_[1], index=ch_names, columns=ch_names)
g = sns.heatmap(df, ax=axes[1], square=True, cbar=True, xticklabels=2,
                yticklabels=2)
...
# display common covariance matrices of two classes (hands, feats)
cov_train0 = []
cov_train1 = []
for k in range(0, len(labels)):
    if(labels[k] == 0):
        cov_train0.append(cov_data_train[k, :, :])
    else:
        cov_train1.append(cov_data_train[k, :, :])

common_cov0 = est_common_cov(cov_train0)
common_cov1 = est_common_cov(cov_train1)

fig, axes = plt.subplots(1, 2, figsize=[8, 4])
ch_names = [ch.replace('.', '') for ch in epochs.ch_names]

df = pd.DataFrame(data=common_cov0, index=ch_names, columns=ch_names)
g = sns.heatmap(df, ax=axes[0], square=True, cbar=True, xticklabels=2,
                yticklabels=2)
g.set_title('common covariance - hands')

df = pd.DataFrame(data=common_cov1, index=ch_names, columns=ch_names)
g = sns.heatmap(df, ax=axes[1], square=True, cbar=True, xticklabels=2,
                yticklabels=2)
plt.xticks(rotation='vertical')
plt.yticks(rotation='horizontal')
g.set_title('common covariance - feets')
